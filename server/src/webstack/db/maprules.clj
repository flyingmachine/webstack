(ns webstack.db.maprules
  (:require [webstack.db.query :as db]
...
(defn sorted-content
  ...
  #(sort-by sort-fn (webstack.db.query/all content-attribute [:content/author (:db/id %)])))
...
(defmaprules ent->post
  ...
  (has-one :author
           :rules webstack.db.maprules/ent->user
           :retriever :content/author)
  (has-one :topic
           :rules webstack.db.maprules/ent->topic
...
(defmaprules ent->user
  ...
  (has-many :topics
            :rules webstack.db.maprules/ent->topic
            :retriever #(webstack.db.query/all :topic/title [:content/author (:db/id %)]))
  (has-many :posts
            :rules webstack.db.maprules/ent->post
            :retriever (webstack.db.maprules/sorted-content :post/content :post/created-at)))
